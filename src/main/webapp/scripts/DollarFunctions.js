...
{
  var fieldValue = field.value.noCommas ();
...
function totalFields (totalField) // second and subsequent arguments are fields
{
  ...
  for (var i = 1; (i < arguments.length) && ok; i++)
  {
    ...
    {
      var fieldValue = parseInt (field.value.noCommas (),10);
      if (field.value != fieldValue.comma ())
        field.value = fieldValue.comma ();
      if (negating)
        total -= fieldValue;
      else
        total += fieldValue;
